...
function Policy(){
	...
	this.policyShut=function(data,policy){ //是否开启了差旅政策
		...
		if(airPolicy===201){
			...
			$(".tipsChailv .tip-resault").html("暂未启用差旅政策");
			if(this.quanxian==1){
				$(".tipsChailv a").html("启用差旅政策>>");
				$("a[data-flag='crm']").parents(".tab").removeClass("hide");
			}else{
				$(".tipsChailv a").html("您没有权限启用差旅政策").removeAttr("href");
	...
	this.airShowPolicy=function(data){
		if(data===2021){ //判断是否设置了差旅政策
			var setText="";
			if(this.quanxian==1){
				setText='<p><a href="/crm/chailv/" class="color-6461e2">前往设置>></a></p>';
				$("a[data-flag='crm']").parents(".tab").removeClass("hide");
			}else{
				setText='<p><a class="color-6461e2">您没有权限设置差旅政策</a></p>';
				$("a[data-flag='crm']").parents(".tab").remove();
			}
			$(".board-air .board-c").html('<p>暂未设置差旅政策</p>'+setText);
			$(".board-air").remove();
			return;
		}
		if(data===202){ //判断是否设置了差旅政策
			...
			$(".board-air .board-c").html('<p>该职级暂未设置差旅政策</p>'+setText);
			return;
		}
	...
	this.trainShowPolicy=function(data){
		if(data===2021){ //判断是否设置了差旅政策
			...
			$(".board-train .board-c").html('<p>暂未设置差旅政策</p>'+setText);
			return;
		}
		if(data===202){ //判断是否设置了差旅政策
			var setText="";
			...
			// 	$("a[data-flag='crm']").parents(".tab").remove();
			// }
			// $(".board-train .board-c").html('<p>该职级暂未设置差旅政策</p>'+setText);
			$('.board-train ').remove();
			return;
		}
	...
	this.hotelShowPolicy=function(data){
		...
		if(data===202){ //判断是否设置了差旅政策
			...
			// 	$("a[data-flag='crm']").parents(".tab").remove();
			// }
			// $(".board-hotel .board-c").html('<p>暂未设置差旅政策</p>'+setText);
			$(".board-hotel").remove();
			return;
		}
		if(policy===null){ //判断某个职级是否设置了差旅政策
			...
			// 	$("a[data-flag='crm']").parents(".tab").remove();
			// }
			// $(".board-hotel .board-c").html('<p>该职级暂未设置差旅政策</p>'+setText);
			$(".board-hotel").remove();
			return;
		}
