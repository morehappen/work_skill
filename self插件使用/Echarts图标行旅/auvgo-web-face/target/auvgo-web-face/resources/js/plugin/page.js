...
pageNav.nav = function(p, pn) {
    ...
    if (p <= 1) {
        ...
        //非第一页
        re += this.pHtml(p - 1, pn, "上一页","class='prev'");
        //总是显示第一页页码
    ...
    if (p < pn) {
        re += this.pHtml(p + 1, pn, "下一页","class='next'");
    };
    return re;
};
//显示非当前页
pageNav.pHtml = function(pageNo, pn, showPageNo,show) {
    showPageNo = showPageNo || pageNo;
    show=show!=undefined?show:'';
    var H = "<li><a "+show+" href='javascript:pageNav.go(" + pageNo + "," + pn + ");'>" + showPageNo + "</a></li> ";
    return H;

};
//显示当前页
pageNav.pHtml2 = function(pageNo) {
    var H = '<li><a class="active" href="javascript:;">'+pageNo+'</a></li>';
    return H;
};

//输出页码,可根据需要重写此方法
pageNav.page= function(p, pn) {
