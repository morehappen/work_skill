...
function kkpagerMain(data){
	kkpager.generPageHtml({
		pno:data.pageNo.val(),  // 当前页数
		total:data.totalPage.val(), // 总页数
		totalRecords:data.totalRecords.val(), // 总条数
		mode: data.mode, // 模式(link 或者 click) 
		click: function(n){  // 点击页数回调函数
			data.pageNo.val(n);
			data.submit_element.submit();
		}
	});
	(function(){
		var content="<span style='padding-left:8px'>每页显示<input type='text' value='"+data.pageSize.val()+"' class='kkpager_btn_go_input' id='everyNums'><button class='btn btn-default btn-small' id='changePages' type='button' style='vertical-align: baseline;'>确定</button></span>"
		$(".infoTextAndGoPageBtnWrap").append(content);
	})();
	$("body").on("click","#changePages",function(){
		var size=$("#everyNums").val();
	...
	$("body").on("blur","#everyNums",function(){
		var psize = $(this).val();
		if(parseInt(psize)>50){
			psize = 15;
		}
		data.pageSize.val(psize);
		data.pageNo.val(1);
	});
}