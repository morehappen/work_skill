...
function updateEmp (data) {
	...
	$.each(data, function (index, curr) {
		html +=
			...
			(curr.linkEmpid!=0?'<span class="linkEmpid " >常用</span>':"")+'<span class="p-name hoverTips hover_content " >' + curr.name + '</span>' +
...
function updatePassList ($dom, matchPolicyCallback, isAppend) {
	...
	$dom.each(function (index) {
		...
		html +=
			'<div class="e-p-model font-size-12   border-top-dashed" style="border-bottom: 1px dashed #e5dfff;" ' +
			'data-id="' + data.id + '" ' +
			...
			'<span class="p-every-passes text-ellipsis" title="' + (empName?empName:"") + '">' + (empName?empName:"") + '</span>' +
			'</span>' +
			'<div class="float-left clear">' +
			(canEdit==1? '<span class="edit_p"></span>' : '') +
			'<span class="remove_p"></span>' +
			(modelFlag == 'airOrder'? '<div class="label label-checkbox cursor notice-margin margin-top-8 margin-left-250 '+(data.mobile?"label-select-checkbox":"show_choice_default")+'  send_sms_pass_d" data-id="'+data.id+'" >' +
			'<span class="show_choice"></span>' +
			'<input type="hidden" name="airUser['+index+'].isSend" data-is="isSend-isSendEmail" checked="checked" value="'+(data.mobile?1:0)+'">' +
			'<span>短信</span>' +
			'</div>' +
			'<div class="label label-checkbox cursor notice-margin margin-top-8 '+(data.email?"label-select-checkbox":"show_choice_default")+' send_eml_pass_d" data-id="'+data.id+'" >' +
			'<span class="show_choice"></span>' +
			'<input type="hidden" name="airUser['+index+'].isSendEmail"  data-is="isSend-isSendEmail" checked="checked" value="'+(data.email?1:0)+'">' +
			'<span>邮件</span>' +
			'</div>' :'<div class="label label-checkbox cursor notice-margin margin-top-8  margin-left-250 '+(data.mobile?"label-select-checkbox":"show_choice_default")+' send_sms_pass_d" data-id="'+data.id+'" >' +//火车票模块
			'<span class="show_choice"></span>' +
			'<input type="hidden" name="users['+index+'].isSend" data-is="isSend-isSendEmail" checked="checked" value="'+(data.mobile?1:0)+'">' +
			'<span>短信</span>' +
			'</div>' +
			'<div class="label label-checkbox cursor notice-margin margin-top-8 '+(data.email?"label-select-checkbox":"show_choice_default")+' send_eml_pass_d" data-id="'+data.id+'" >' +
			'<span class="show_choice"></span>' +
			'<input type="hidden" name="users['+index+'].isSendEmail"  data-is="isSend-isSendEmail" checked="checked" value="'+(data.email?1:0)+'">' +
			'<span>邮件</span>' +
			'</div>')+
			'</div>' +
			'</div>' +
        ...
        if(projectinfo==1||projectinfo==3){
			...
			'<div class="float-left drop-centro p-w-160" data-value="true">' +
        ...
        if (modelFlag == 'airOrder') {//机票模块
            html +=
                ...
                '<input style="border: 0px" type="text" name="airUser['+_index+'].certno" value="' + (defCert.certNo?defCert.certNo:"") + '" class="input user_centro" readonly  datatype="*" nullmsg="证件号不能为空">' +
                '</div>' ;
            if(costcenter ==1||costcenter ==3) {
