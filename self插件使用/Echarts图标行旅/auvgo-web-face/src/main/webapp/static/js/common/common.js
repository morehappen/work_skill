...
(function () {
    ...
    if (!($(".guoji-needs").is("div")) && pathname) {
...
//		console.log(window.location.pathname);
        ...
        if(flag_check == 'caiwu'){
            $(".sub-nav a[href^='/caiwu']").addClass("sub-active");
            return;
        }
        if(tag_check=='hotel' || tag_check=='hotelPay'){
            $(".sub-nav a[href^='/myChailv']").addClass("sub-active");
            return;
        }else if(flag_check == 'myApproval'){
            $(".sub-nav a[href^='/myApproval']").addClass("sub-active");
            return;
        }
        ...
        $('[data-flag="air"]').addClass("sub-active");
    }
})();
//运行下拉主函数
...
function ztreeMain(data) {
    $(function () {
        ...
        var setting = {
            view: {dblClickExpand: false, showLine: false, selectedMulti: false}, data: {simpleData: {enable: true}},
            callback: {
...
function OuterInterface() {
    ...
    if (this.getOuterInterface !== "function") {
        ...
        OuterInterface.prototype.clearPower = function (flag) {
            // console.log(flag);
            if(flag!="air"){
				$(".sub-nav-ul [data-flag='air']").remove();
            }
...
//判断第三方登录过来的时候导航栏
	if($('.casModel_flag').val()!=""&&$('.casModel_flag')>0){
        var casModel = JSON.parse($('.casModel_flag').val());
        if(casModel.product!="air"){
            $('.sub-nav-ul [data-flag="air"]').remove();
        }
	}
