...
function outer_shenpi (data,flag) {
	...
	$.each(level,function(index,item){
		...
		if(index < xuhao){
			eLevelList='<div class="clear"><div class="sp-jb float-left line-height-48">'+(item)+'级审批</div><div class="sp-jb float-left">'+eLevelList+ '</div></div>';
			...
			addPerson = function (data) {
				...
				$.each(data,function(index,item){
					...
					person += '<div class="passanger-model" ' +
						...
						'<span class="float-left p-m-name clear position"><span class="icon-img pass-bg position-ab hide"></span>' +
...
$("#train-book-form").Validform({
	...
	ajaxPost: (interface.getOuterInterface()?true:false),
